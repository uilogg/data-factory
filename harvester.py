...
def generer_page_web_seo(article_html):
  print("[+] Étape 3 : Génération de la page web statique (index.html) version Haute-Gamme...")
  date_du_jour = datetime.now().strftime("%d/%m/%Y")
...
<body class="bg-slate-950 text-slate-100 antialiased selection:bg-blue-500 selection:text-white">
    ...
    <header class="relative border-b border-slate-800/60 bg-slate-900/40 backdrop-blur-xl sticky top-0 z-50">
        <div class="max-w-5xl mx-auto px-6 h-20 flex items-center justify-between">
            ...
            <a href="{GUMROAD_PRODUCT_URL}" target="_blank" class="hidden sm:inline-flex items-center px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold transition-all shadow-lg shadow-blue-600/20">
                Télécharger la base
            </a>
        </div>
    </header>
    ...
    <section class="relative pt-20 pb-12 overflow-hidden">
        <div class="max-w-4xl mx-auto px-6 text-center relative z-10">
            <div class="inline-flex items-center px-3.5 py-1.5 rounded-full text-xs font-semibold bg-blue-500/10 text-blue-400 border border-blue-500/20 mb-6 shadow-inner">
                <span class="w-2 h-2 rounded-full bg-blue-400 animate-ping mr-2"></span> Rapport Hebdomadaire — Semaine du {date_du_jour}
            </div>
            <h1 class="text-4xl sm:text-6xl font-extrabold tracking-tight mb-6 leading-[1.1]">
                Boostez vos ventes avec des <br/><span class="bg-gradient-to-r from-blue-400 via-indigo-300 to-purple-400 bg-clip-text text-transparent">Leads B2B ultra-qualifiés</span>
            </h1>
            <p class="text-slate-400 text-lg sm:text-xl max-w-2xl mx-auto mb-10 leading-relaxed">
                Accédez chaque semaine aux signaux d'affaires en France : levées de fonds, créations d'entreprises et recrutements prêts pour votre CRM.
            </p>
        </div>
    </section>
    ...
    <section class="max-w-4xl mx-auto px-6 pb-12">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div class="bg-slate-900/60 border border-slate-800/80 rounded-2xl p-6 backdrop-blur-md text-center">
                <div class="text-3xl font-extrabold text-indigo-400 mb-1">CRM-Ready</div>
                <div class="text-xs text-slate-400 font-medium uppercase tracking-wider">Format CSV Structuré</div>
            </div>
            <div class="bg-slate-900/60 border border-slate-800/80 rounded-2xl p-6 backdrop-blur-md text-center">
                <div class="text-3xl font-extrabold text-purple-400 mb-1">Hebdomadaire</div>
                <div class="text-xs text-slate-400 font-medium uppercase tracking-wider">Actualisation Automatique</div>
            </div>
        </div>
    </section>
...
"""
  ...
  print("[SUCCÈS] Page web 'index.html' ultra-design générée avec succès !")
